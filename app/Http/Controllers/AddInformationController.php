...
class AddInformationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $riskNums = $request->input('risk_num', []); // Array ของ risk_num
            $riskDetails = $request->input('risk_detail', []); // Array ของ risk_detail

            // สร้าง array สำหรับจัดเก็บข้อมูล
            $riskArray = [];

            foreach ($riskNums as $index => $riskId) {
                $riskDetail = $riskDetails[$index] ?? ''; // ใช้ค่าเริ่มต้นถ้าไม่มีข้อมูล
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $riskNumArray = [];    // Array สำหรับเก็บ risk_num
            $riskDetailArray = []; // Array สำหรับเก็บ risk_detail

            // ดึงข้อมูลทั้งหมดของ general_information_id ที่เกี่ยวข้อง
            $existingRisks = AssessPatientRisk::where('general_information_id', $general_information->id)->pluck('id', 'risk_num')->toArray();

            foreach ($riskNums as $index => $riskId) {
                $riskDetail = $riskDetails[$index] ?? ''; // ใช้ค่าเริ่มต้นถ้าไม่มีข้อมูล

                // อัปเดตหรือสร้างข้อมูลในฐานข้อมูล
                $record = AssessPatientRisk::updateOrCreate(
                    [
                        'general_information_id' => $general_information->id, // เงื่อนไขที่ใช้ค้นหา
                        'risk_num' => $riskId,
                    ],
                    [
                        'risk_detail' => $riskDetail, // ข้อมูลที่ต้องการอัปเดตหรือเพิ่ม
                    ]
                );

                // เก็บข้อมูลที่ถูกอัปเดตหรือสร้างใหม่
                $riskNumArray[] = $record->risk_num;
                $riskDetailArray[] = $record->risk_detail;

                // ลบ risk_num ที่พบในคำขอออกจากรายการที่ต้องลบ
                unset($existingRisks[$riskId]);
            }

            // ลบข้อมูลที่เหลือใน $existingRisks ซึ่งไม่ได้อยู่ในคำขอ
            AssessPatientRisk::whereIn('id', array_values($existingRisks))->delete();
